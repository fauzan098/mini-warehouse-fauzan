...
func (h *FileUploadHelper) UploadPhoto(ctx context.Context, file *multipart.FileHeader) (*UploadResult, error) {
	...
	result, err := h.storege.UploadFile(ctx, file, "users")
	if err != nil {
...
func (h *FileUploadHelper) validateImageFile(file *multipart.FileHeader, maxSize int64) error {
	if !validateFileZise(file.Size, maxSize) {
...
func validateFileZise(size int64, maxSize int64) bool {
	return size <= maxSize
}
...
func validateFileExtension(extension string, allowedExtension string) bool {
	allowed := strings.Split(allowedExtension, ",")
