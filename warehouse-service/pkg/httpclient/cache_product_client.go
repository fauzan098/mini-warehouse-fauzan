...
func NewCacheProductClient(productClient ProductClientInterface, redisClient *redis.RedisClient, ttl time.Duration) *CachedProductClient {
...
func (cpc *CachedProductClient) GetProductByID(ctx context.Context, productID uint) (*ProductResponse, error) {
	...
	var cacheProduct ProductResponse
	if err := cpc.redis.Get(ctx, cacheKey, &cacheProduct); err == nil {
		log.Infof("[CacheProductClient] GetProductByID - 1: %v", cacheProduct)
