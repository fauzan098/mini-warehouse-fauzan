...
type RabbitMQServiceInterface interface {
	PublisEmail(ctx context.Context, payload EmailPayload) error
	Close() error
}
...
// PublisEmail implements [RabbitMQServiceInterface].
func (r *rabbitMQService) PublisEmail(ctx context.Context, payload EmailPayload) error {
	...
	// publish email queue langsung (tanpa exchange)
	err = r.ch.Publish(
	...
	log.Infof("Email message publish for user %d: %s", payload.UserID, payload.Type)
	return nil
}
